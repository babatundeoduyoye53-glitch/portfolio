...
export default function WhatsAppButton() {
  ...
  const message = encodeURIComponent("Hi Tunice! I saw your portfolio and I'd love to connect.")
  const href = `https://wa.me/${meta.whatsapp}?text=${message}`
  ...
  return (
    <a
      ...
      "
    >
      {/* Tooltip — desktop only, slides in from left */}
      <div
        className="hidden md:block font-body text-xs font-medium px-3 py-1.5 rounded-full whitespace-nowrap
          transition-all duration-200 pointer-events-none"
        style={{
          background: '#25D366',
          color: '#ffffff',
          opacity: hovered ? 1 : 0,
          transform: hovered ? 'translateX(0) scale(1)' : 'translateX(-6px) scale(0.95)',
